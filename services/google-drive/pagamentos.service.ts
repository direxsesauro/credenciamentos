...
import { PagamentoFinanceiro } from '../../types';

// Cache para armazenar dados do CSV
interface CacheData {
  data: PagamentoFinanceiro[];
  timestamp: number;
}
...
async function fetchCSVFromDrive(): Promise<string> {
  ...
  const fileIdInput = import.meta.env.VITE_GOOGLE_DRIVE_PAGAMENTOS_FILE_ID;
  ...
  if (!apiKey || !fileIdInput) {
    ...
    if (!fileIdInput) missingVars.push('VITE_GOOGLE_DRIVE_PAGAMENTOS_FILE_ID');
  ...
  try {
    ...
    console.log('CSV de pagamentos carregado com sucesso do Google Drive. Tamanho:', csvText.length, 'caracteres');
...
/**
 * Parseia o CSV e retorna array de PagamentoFinanceiro
 */
function parseCSV(csvText: string): Promise<PagamentoFinanceiro[]> {
  return new Promise((resolve, reject) => {
    Papa.parse(csvText, {
      ...
      complete: (results) => {
        try {
          const pagamentos: PagamentoFinanceiro[] = [];
          ...
          if (results.data.length > 0) {
            ...
            console.log('Colunas encontradas no CSV de pagamentos:', columns);
          }

          for (const row of results.data as any[]) {
            // Verificar se a linha tem o campo DOCUMENTO (número da OB)
            const documento = getColumnValue(row, 'DOCUMENTO');
            if (!documento || !documento.trim()) {
              continue;
            }

            // Filtrar apenas linhas que são ordens bancárias (contêm "OB" no documento)
            const documentoNormalizado = documento.trim().toUpperCase();
            if (!documentoNormalizado.includes('OB')) {
              continue;
            }

            // Extrair as colunas do pagamento
            const pagamento: PagamentoFinanceiro = {
              documento: documentoNormalizado,
              data: getColumnValue(row, 'DATA') || '',
              numero_empenho: getColumnValue(row, 'DOC.ORIG.NE') || '',
              valor: parseValue(getColumnValue(row, 'MOVIMENTO')),
              unidade: getColumnValue(row, 'UNIDADE') || undefined,
              fonte_recurso: getColumnValue(row, 'FONTE DE RECURSO') || undefined,
              despesa: getColumnValue(row, 'DESPESA') || undefined,
              numero_processo: getColumnValue(row, 'NUMERO PROCESSO') || undefined,
              credor: getColumnValue(row, 'CREDOR') || undefined,
              historico: getColumnValue(row, 'HISTORICO') || undefined,
            };

            pagamentos.push(pagamento);
          }

          console.log(`Total de pagamentos processados: ${pagamentos.length}`);
          resolve(pagamentos);
        } catch (error) {
...
/**
 * Busca todos os pagamentos do Google Drive (com cache)
 */
export async function fetchPagamentosFromDrive(): Promise<PagamentoFinanceiro[]> {
  // Verificar cache
  ...
  const pagamentos = await parseCSV(csvText);
...
/**
 * Busca um pagamento específico pelo número da ordem bancária
 */
export async function getPagamentoByOB(numeroOB: string): Promise<PagamentoFinanceiro | null> {
  if (!numeroOB || !numeroOB.trim()) {
    return null;
  }

  const numeroNormalizado = numeroOB.trim().toUpperCase();
  console.log('Buscando pagamento com OB normalizado:', numeroNormalizado);

  const pagamentos = await fetchPagamentosFromDrive();
  console.log(`Total de pagamentos carregados: ${pagamentos.length}`);
  
  // Debug: mostrar alguns exemplos de documentos
  if (pagamentos.length > 0) {
    console.log('Exemplos de documentos no CSV:', pagamentos.slice(0, 5).map(p => p.documento));
  }

  const encontrado = pagamentos.find(p => p.documento === numeroNormalizado);
  
  if (encontrado) {
    console.log('Pagamento encontrado:', encontrado);
  } else {
    console.warn(`Pagamento não encontrado para OB: ${numeroNormalizado}`);
    // Tentar busca parcial (caso o usuário tenha digitado sem alguns caracteres)
    const parcial = pagamentos.find(p => p.documento.includes(numeroNormalizado) || numeroNormalizado.includes(p.documento));
    if (parcial) {
      console.log('Encontrado match parcial:', parcial);
    }
  }
...
/**
 * Busca múltiplos pagamentos pelos números das ordens bancárias
 */
export async function getPagamentosByOBs(numerosOB: string[]): Promise<PagamentoFinanceiro[]> {
  if (!numerosOB || numerosOB.length === 0) {
    return [];
  }

  const pagamentos = await fetchPagamentosFromDrive();
  const numerosNormalizados = numerosOB.map(n => n.trim().toUpperCase()).filter(n => n);

  return pagamentos.filter(p => numerosNormalizados.includes(p.documento));
}
